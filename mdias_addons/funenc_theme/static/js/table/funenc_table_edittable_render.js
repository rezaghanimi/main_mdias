odoo.define('funenc.EditableListRenderer', function (require) {
    "use strict";
    ...
    ListRenderer.include({
        ...
        events: _.extend({}, ListRenderer.prototype.events, {
            ...
            'click tbody div.o_data_cell': '_onCellClick',
        ...
        setRowMode: function (recordID, mode) {
            ...
            _.each(this.columns, function (node, colIndex) {
                ...
                if (editMode) {
                    $td.parent().empty().append($newTd.contents());
                } else {
                    self._unregisterModifiersElement(node, recordID, $td);
                    $td.parent().replaceWith($newTd);
                }
            });
            delete this.defs;

            // Destroy old field widgets
            oldWidgets = _.filter(oldWidgets, function(widget) {   
                if (widget.$el.parents(".o_fixed_row").length > 0) {
                    return false;
                }
                return true;
            })
        ...
        _selectCell: function (rowIndex, fieldIndex, options) {
            ...
            return this._selectRow(rowIndex).then(function () {
                ...
                var recordWidgets = _.filter(self.allFieldWidgets[record.id] || [], function(widget) {
                    return !widget.fixed
                })

                if (fieldIndex >= recordWidgets.length) {
        ...
        _onCellClick: function (event) {
            ...
            if (!this._isEditable() || $(event.target).prop('special_click') || $(event.target).attr('type') == 'action') {
                return;
            }
        ...
        _onWindowClicked: function (event) {
            ...
            var real_table = this.$('.layui-table-body.layui-table-main.layui-table-body-main>table')
            if (real_table[0].contains(event.target) && real_table[0] !== event.target) {
                return;
            }
