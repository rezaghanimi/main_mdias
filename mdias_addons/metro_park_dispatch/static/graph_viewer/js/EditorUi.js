 ...
 */
EditorUi = function (editor, container, lightbox, defaultUrl) {
	...
	if (!graph.standalone) {
		...
		var styles = ['rounded', 'shadow', 'glass', 'dashed',
			'dashPattern', 'comic', 'labelBackgroundColor'];

		var connectStyles = ['shape', 'edgeStyle', 'curved',
			'rounded', 'elbow', 'comic', 'jumpStyle', 'jumpSize'];
			...
			try {
				...
				if (state != null) {
					...
					this.fireEvent(new mxEventObject(
						'styleChanged', 'keys', keys, 'values', values, 'cells', [state.cell]));
				}
			}
			catch (e) {
				this.handleError(e);
 ...
 */
EditorUi.prototype.footerHeight = 25;
 ...
 */
EditorUi.prototype.open_xml_model = function (node) {
	try {
		// Fixes possible asynchronous requests
		try {
			this.editor.setGraphModelXml(node)
...
EditorUi.prototype.open_url_file = function (url) {
	// Cross-domain window access is not allowed in FF, so if we
	// were opened from another domain then this will fail.
	try {
		// Fixes possible asynchronous requests
		var self = this
		mxUtils.get(url, function (xhr) {
			// Adds bundle text to resources
			mxResources.parse(xhr.getText());
			try {
				self.editor.setGraphModelXml(xhr.getDocumentElement())
