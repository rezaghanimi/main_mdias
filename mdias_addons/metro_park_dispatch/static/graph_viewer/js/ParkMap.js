 ...
 * 通过option传入相关数据
 */
ParkMap = function (editorUi, netWork, options) {
...
mxUtils.extend(ParkMap, mxEventSource);

ParkMap.prototype.initMenuActions = function () {
    var self = this
    // 占线板删除
...
ParkMap.prototype.parseButtonTable = function () {
...
ParkMap.prototype.hookGetCursorForCell = function () {
 ...
 * 元素点击事件
 */
ParkMap.prototype.initParkElementClick = function () {
...
// 获取cell
ParkMap.prototype.getCellUid = function (cell) {
 ...
 * 确定按扭点击
 */
ParkMap.prototype.onConfirmClick = function (parkElement, uindex, eventTarget) {
    var self = this
    pop.confirm({
        title: "提示",
        sizeAdapt: false,
 ...
 * 自控模式点击
 */
ParkMap.prototype.onSelfControlClick = function (bSelfControl) {
    if (this.disableOperation) {
        return
    }
    var self = this
...
// 计时函数
ParkMap.prototype.startCounting = function () {
    var self = this
 ...
 * 设置全局状态
 */
ParkMap.prototype.setGlobalState = function (state) {
    ...
    // 通信状态
        ...
        if (state.data.type) { // 联锁
            ...
            if (state.data.status == 1) {
                ...
                this.fireEvent(new mxEventObject('add_interlock_alarm', 'msg', 'MDIAS与联锁通信中断!'))
                ...
                if (this.frontState == 0) {
                    ...
                    this.fireEvent(new mxEventObject('add_interlock_alarm', 'msg', 'MDIAS与联锁通信恢复!'))
                }
            } else if (state.data.status == 2) {
                this.fireEvent(new mxEventObject('show_block'))
                this.fireEvent(new mxEventObject('add_interlock_alarm', 'msg', '联锁机器故障!'))
            ...
            if (state.data.status == 1) {
                ...
                this.fireEvent(new mxEventObject('show_block'))
                this.fireEvent(new mxEventObject('add_interlock_alarm', 'msg', '前置机机器故障'))
            } else if (state.data.status == 3) {
                this.fireEvent(new mxEventObject('show_block'))
...
ParkMap.prototype.normalizeUID = function (uid) {
    return uid.replace('/', '_').toUpperCase()
}

// 设置区段状态
ParkMap.prototype.setRailSecStatus = function (uid, status) {
    ...
    if (!status.hold && status.lock) {
        roads.map(road => {
            this.setFillColor(road, this.railColors.white)
        })
    }
...
//设置状态灯状态
ParkMap.prototype.setLightStatus = function (uid, status) {
...
ParkMap.prototype.showcell = function (cell) {
    if (cell && cell.setVisible) {
        this.graph.model.setVisible(cell, true)
    }
}

ParkMap.prototype.setLabelText = function (cell, code) {
 ...
 * 设置cell的填充颜色
 */
ParkMap.prototype.setFillColor = function (cell, color, isFlash) {
 ...
 * 设置样式上的内容
 */
ParkMap.prototype.setCellStyle = function (cell, key, value) {
...
// 换cell的边框颜色
ParkMap.prototype.setStrokeColor = function (cell, color) {
    if (!cell) {
        return
    }
...
// 全局闪烁
ParkMap.prototype.DoFlashCells = function () {
 ...
 * 取得分组index
 */
ParkMap.prototype.getGroupIndex = function (uid, x, y) {
    var self = this
...
ParkMap.prototype.getCellPostion = function (cell) {
...
ParkMap.prototype.getGroupOffset = function (uid, group_index) {
    var x = 0
    if (uid in this.uidTrainCache) {
        var train_ids = this.uidTrainCache[uid]
...
// 添加容器
ParkMap.prototype.addTrain = function (status) {
 ...
 * 删除
 */
ParkMap.prototype.removeCarriage = function (train_no) {
...
// 添加容器
ParkMap.prototype.addCarriage = function (status) {
 ...
 * 取得区段的中间位置
 */
ParkMap.prototype.getSecCenterPosition = function (uid) {
    uid = uid.toUpperCase();
...
ParkMap.prototype.initTrainStyle = function () {
...
ParkMap.prototype.initBusyIconStyle = function () {
...
ParkMap.prototype.dualMap = function (parts, func) {
    parts.map(part => {
        part.map(func)
    })
}
...
// 设置道岔状态
ParkMap.prototype.setSwitchStatus = function (uid, status) {
    ...
    if (status.hold == 0 && status.lock == 1) {
        ...
        parts.map(part => {
            part.map((cell) => {
                this.setFillColor(cell, this.switchColors.white)
            })
        })
    }
    ...
    // 闭锁状态
    if (status.closed) {
        nameLabels.map(nameLabel => this.setLabelText(nameLabel,
            '<div style="border:1px solid #f00">' + nameLabel.getAttribute('label') + '</div>'))
    }
...
// 设置信号机状态
ParkMap.prototype.setSignalStatus = function (uid, status) {
    ...
    if (status.delay_30s || status.delay_180s) {
        ...
        var remain = status.delay_30s ? 30 : 180
        var interval = setInterval(() => {
            ...
            if (!cell.elementStatus.delay_30s && !cell.elementStatus.delay_180s) {
                condition = true
            }
            if (condition) {
...
// 重置状态
ParkMap.prototype.resetStatus = function () {
    this.startTime = null
    this.status = 0
...
// 按钮点击处理
ParkMap.prototype.buttonClick = function (btnInfo, button, e) {
    var self = this
            ...
            if (btnInfo.cell.elementStatus.red_white ||
                ...
                if (button && button.type && (button.type == 'la' || button.type == 'ya')) {
                    ...
                    self.fireEvent(new mxEventObject(
                        'add_warning',
                        'msg',
                        `${this.operationName[this.status]} ${btnInfo.cell.elementStatus.name}`));
                    self.commitAction()
                    return
                }
                ...
                if (button && button.type && (button.type == 'la' || button.type == 'da')) {
                    ...
                    //     'add_warning',
                    //     'msg',
                    //     `${this.operationName[this.status]} ${equip.cell.elementStatus.name}`));
                    self.commitAction()
                    return
                }
            }
        ...
        case 11: {
            ...
            if (btnInfo.type == 'ca') {
                ...
                this.fireEvent(new mxEventObject(
                    'add_warning',
                    'msg',
                    `${btnInfo.cell.elementStatus.name} ${this.operationName[this.status]}`));
                self.commitAction()
                return
            }
            break
...
// 发送命令并重置状态
ParkMap.prototype.commitAction = function () {
    switch (this.status) {
        case 1:
            this.status = 0x05
...
ParkMap.prototype.clearOldTrain = function (uid) {
    if (!this.parkElementCache[uid]) {
        return
    }
    var parentCell = this.parkElementCache[uid];
...
ParkMap.prototype.AddTrainGroups = function (trainInfos) {
    var self = this
 ...
 * 更新现车位置
 */
ParkMap.prototype.changeTrainPosition = function (trainInfo) {
    var self = this
 ...
 */
ParkMap.prototype.initTrainPosition = function (groupInfos) {
    var def = $.Deferred();
    var self = this;
...
ParkMap.prototype.initBusyIcons = function () {
    var self = this;
    var location = this.location
    location = location.replace("_", "")
    this.netWork.call_server_method(
...
ParkMap.prototype.SetBusyIconStatus = function (status) {
...
ParkMap.prototype.ShowBusyIcons = function (show) {
...
ParkMap.prototype.updateSwitches = function () {
    this.gloabalUpdate = true
 ...
 * 显示隐藏道岔区段名称
 */
ParkMap.prototype.UpdateSwitchSectionName = function () {
    this.gloabalUpdate = true
    this.model.beginUpdate()
    try {
        // 在区故解时显示全部区段
 ...
 * 刷新信号机
 */
ParkMap.prototype.UpdateSignal = function () {
    this.gloabalUpdate = true
    this.model.beginUpdate()
    try {
        for (var key in this.parkElementCache) {
 ...
 * 刷新无道岔区段
 */
ParkMap.prototype.udpateRailSec = function () {
    this.gloabalUpdate = true
    this.graph.model.beginUpdate()
    try {
        for (var key in this.parkElementCache) {
...
ParkMap.prototype.executeInstruct = function (state) {
...
ParkMap.prototype.shadingSignal = function (state) {
...
ParkMap.prototype.sendControlPlan = function () {
    if (this.disableOperation) {
        return
    }
...
ParkMap.prototype.showTrains = function (bShow) {
...
ParkMap.prototype.toggle_train_orders = function (bShow) {
    var cells = []
    for (var uid in this.trainCache) {
        var train = this.trainCache[uid]
        cells.push(train)
