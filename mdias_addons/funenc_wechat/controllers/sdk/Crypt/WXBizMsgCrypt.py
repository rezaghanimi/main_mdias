...
class Prpcrypt(object):
    ...
    def encrypt(self, text, corpid):
        ...
        """
        # 16位随机字符串添加到明文开头
        len_str = struct.pack("I", socket.htonl(len(text.encode('utf-8'))))
        text = self.get_random_str() + binascii.b2a_hex(len_str).decode('utf-8') + text.encode('utf-8') + corpid
        # 使用自定义的填充方式对明文进行补位填充
        pkcs7 = PKCS7Encoder()
...
class WXBizMsgCrypt(object):
    ...
    def __init__(self, sToken, sEncodingAESKey, sCorpId):
        ...
        self.m_sToken = sToken.encode('utf-8')
        self.m_sCorpid = sCorpId.encode('utf-8')
