...
class ImportTimeTable(models.Model):
    ...
    def on_ok(self):
        ...
        main_line = self.env.ref("metro_park_base_data_8.main_line_location").id
        gao_da_lu = self.env.ref("metro_park_base_data_8.gao_da_lu").id
        ban_qiao = self.env.ref("metro_park_base_data_8.ban_qiao").id
        ...
        if self.type == 'run_plan':
            ...
            for row in valid_rows:
                ...
                if out_location in location_cache or back_location in location_cache:
                    ...
                    try:
                        ...
                        _logger.info('import time table error {e}'.format(e=e))
                        print(e)
            ...
            for i in range(1, sheet.nrows):
                ...
                if data[4] == "高大路停车场":
                    out_location = gao_da_lu
                elif data[4] == "板桥车辆段":
                    out_location = ban_qiao
                else:
                    out_location = main_line
                if data[5] == "高大路停车场":
                    back_location = gao_da_lu
                elif data[5] == "板桥车辆段":
                    back_location = ban_qiao
                else:
                    back_location = main_line
