...
class BusyBoard(models.Model):
    ...
    def update_construction_icons(self):
        ...
        rails = self.env['metro_park_base.rails_sec'].search([('no', 'in', rail_nos)])
        switches = self.env['metro_park_base.switches'].search([('name', 'in', switch_names)])
        for rail in rails:
            self.set_busy_icon_status(rail.no, rail.loation.alias, 'construction')
        for switch in switches:
            self.set_busy_icon_status(switch.name, switch.loation.alias, 'construction')
    ...
    def get_work_area_info(self):
        ...
        work_area_infos = \
            self.env['metro_park_dispatch.construction_area_relation'].browse(work_area_ids)
        rst = {'rails': [], 'switches': []}
